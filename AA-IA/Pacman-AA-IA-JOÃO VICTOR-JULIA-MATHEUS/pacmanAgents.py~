...
class PacmanAgent(game.Agent):
    ...
    def getAction(self, state):
        ...
        _, nextFood = min([(manhattanDistance(state.getPacmanPosition(), food), food) for food in foodList if food not in state.getGhostPositions()]) # identifica a comida mais proxima que sera o objetivo
